...
def inference(data_dir, model_dir, output_dir, args):
    ...
    dataset = TestDataset(img_paths=img_paths, resize=args.resize, mean=args.mean, std=args.std)
...
#         num_workers=8,
    ...
    with torch.no_grad():
        for idx, images in enumerate(loader):
            ...
            pred = model(images)
            pred_mask = pred[0].argmax(dim=-1)
            pred_gender_age = pred[1].argmax(dim=-1)
            final_pred = pred_mask * 6 + pred_gender_age
            preds.extend(final_pred.tolist())
