...
class Efficientnet_B4_divided(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        self.effnet_mask = EfficientNet.from_pretrained('efficientnet-b4', num_classes=3)
        self.effnet_gender_age = EfficientNet.from_pretrained('efficientnet-b4', num_classes=6)

        
    def forward(self, x):
        x1 = self.effnet_mask(x)
        x2 = self.effnet_gender_age(x)
        return x1, x2

# 새로운 image dataset에 pretrain을 하는 class
class Efficientnet_B4_Pretrain(nn.Module):
    def __init__(self):
        super().__init__()
        # self.effnet_mask = EfficientNet.from_pretrained('efficientnet-b4', num_classes=3)
        self.effnet_gender_age = EfficientNet.from_pretrained('efficientnet-b4', num_classes=6)
...
# 새 image dataset에 pretrain된 모델 parameter를 가져와서 적용.
class Efficientnet_B4_Pretrain2(nn.Module):
    def __init__(self, saved_model, device):
        super().__init__()
        model_cls = getattr(import_module("model"), "Efficientnet_B4_Pretrain")
        model = model_cls()
        model.load_state_dict(torch.load(saved_model, map_location=device))
        self.effnet_gender_age = model
        self.effnet_mask = EfficientNet.from_pretrained('efficientnet-b4', num_classes=3)
